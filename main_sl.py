...
from lib.dataloader import cifar10_dataset, get_sl_sampler, gen_cifar10_dataset
import os
from os import path
import time
import shutil
...
# Dataset Parameters
parser.add_argument('-bp', '--base_path', default="/data/zyh/data")
parser.add_argument('--data-path', default="./output/acgan/gen_time:5/")
...
# GPU Parameters
parser.add_argument("--gpu", default="0,1,2,3", type=str, metavar='GPU plans to use', help='The GPU id plans to use')
parser.add_argument("--type", default="acgan", type=str, metavar='GPU plans to use', help='The GPU id plans to use')
...
def main(args=args):
    ...
    if args.dataset == "Cifar10":
        ...
        num_classes = 10
    elif args.dataset == 'GenCifar10':
        dataset_base_path = args.base_path
        test_dataset = cifar10_dataset(dataset_base_path, train_flag=False)
        # gan_dataset_path = path.join(args.base_path, "data/gen_cifar10_{}/".format(args.gan_time))
        gen_dataset_path = args.data_path
        train_dataset = gen_cifar10_dataset(gen_dataset_path)
        sampler_valid, sampler_train = get_sl_sampler(
            torch.tensor(train_dataset.targets, dtype=torch.int32), 500, 10)
        test_dloader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)
        valid_dloader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True,
    ...
    writer_log_dir = "./output/{}/test/train_time:{}".format(args.type, args.train_time)
